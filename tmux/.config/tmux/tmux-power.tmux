...
battery_icon="$(tmux_get '@battery_icon' '⚡')"
show_upload_speed="$(tmux_get @show_upload_speed false)"
show_download_speed="$(tmux_get @show_download_speed false)"
show_web_reachable="$(tmux_get @show_web_reachable false)"
prefix_highlight_pos=$(tmux_get @prefix_highlight_pos)
time_format=$(tmux_get @time_format '%H:%M')
date_format=$(tmux_get @date_format '%a %d %b')

# TC short for Theme-Colour
TC=$(tmux_get '@theme' 'gold')
case $TC in
    'gold' )
        TC='#ffb86c'
        ;;
